// MODULO: controller/calendar.controller.js
// CAPA: Controlador (recibe HTTP, llama el modelo, responde HTTP)
//
// Responsabilidad unica: manejar las peticiones HTTP del calendario.
...
//   DELETE /api/calendar/:id         → eliminarEvento       (solo el instructor que lo creo)
...
// Importamos las funciones del modelo y les damos alias mas descriptivos con 'as'
import {
...
export const getEventosInstructor = catchAsync(async (req, res) => {
    // Sacamos el id del instructor del token JWT que verifyToken adjunto a la peticion
    ...
    // Enviamos el arreglo de eventos al cliente  puede estar vacio si no ha creado ninguno
    return successResponse(res, 'Eventos del instructor obtenidos', eventos);
});

// ── GET /api/calendar/usuario ─────────────────────────────────────────────────
// Devuelve todos los eventos que el instructor asigno al estudiante logueado
export const getEventosUsuario = catchAsync(async (req, res) => {
    // Sacamos el id del estudiante del token JWT que verifyToken adjunto a la peticion
...
export const crearEvento = catchAsync(async (req, res) => {
    ...
    // Validamos que los tres campos obligatorios llegaron en el cuerpo de la peticion
    if (!date || !title || !tipo) {
        // Sin fecha, titulo o tipo no podemos crear el evento correctamente
    ...
    // Si el tipo es 'estudiante', studentId es obligatorio para saber a quien asignarlo
    ...
    // Respondemos con 201 Created y el evento recien guardado que ya incluye el id de MySQL
    return successResponse(res, 'Evento creado correctamente', nuevoEvento, 201);
});

// ── DELETE /api/calendar/:id ──────────────────────────────────────────────────
// Elimina un evento del calendario
// Solo el instructor que lo creo puede eliminarlo  el modelo verifica la autoria
export const eliminarEvento = catchAsync(async (req, res) => {
    // Sacamos el id del evento a eliminar del parametro de la URL
    const { id }       = req.params;
    // Sacamos el id del instructor del token JWT para verificar que es el dueno del evento
    const instructorId = req.usuario.id;

    // Llamamos a removeEvento que busca el evento por id Y por instructor_id
    // Si el evento no existe, o si el instructor no fue quien lo creo, retorna null
    const eliminado = await removeEvento(id, instructorId);

    // Si removeEvento retorno null el evento no existe o el instructor no tiene permiso
    if (!eliminado) {
    ...
    // Llegamos aqui porque el evento fue eliminado exitosamente  null en data porque ya no existe
