// MODULO: controller/notes.controller.js
// CAPA: Controlador (recibe HTTP, llama el modelo, responde HTTP)
//
// Responsabilidad unica: manejar las peticiones HTTP de notas personales (post-its).
// NUNCA contiene logica de negocio  solo recibe req, llama el modelo y responde res.
//
// Endpoints:
...
// Importamos las 3 funciones del modelo que interactuan con la tabla user_notes en MySQL
import { getNotas, createNota, deleteNota } from '../models/notes.model.js';

// ── GET /api/notes ────────────────────────────────────────────────────────────
// Devuelve todas las notas que pertenecen al usuario que esta logueado
// req.usuario.id viene del token JWT que verifyToken adjunta a la peticion
export const obtenerNotas = catchAsync(async (req, res) => {
    ...
    // Enviamos el arreglo de notas al cliente  puede estar vacio si el usuario no tiene notas aun
...
export const crearNota = catchAsync(async (req, res) => {
    // Sacamos el texto y el color del cuerpo de la peticion  color puede llegar undefined
    const { texto, color } = req.body;
    // Validamos que el texto no este vacio ni sea solo espacios en blanco
    if (!texto || texto.trim() === '') {
        // El texto de la nota es el contenido principal  sin el no tiene sentido guardarla
    ...
    // Respondemos con 201 Created y la nota recien guardada que ya incluye el id de MySQL
...
export const eliminarNota = catchAsync(async (req, res) => {
    ...
    // Si deleteNota retorno null, la nota no existe o el usuario no tiene permiso para eliminarla
    if (!eliminada) {
        return errorResponse(res, 'Nota no encontrada o sin permiso', 404);
    }
    // Llegamos aqui porque la nota fue eliminada  enviamos null en data porque ya no existe
