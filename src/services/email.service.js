// MODULO: services/email.service.js
// CAPA: Servicios
//
// Responsabilidad unica: configurar y enviar correos con Mailtrap via SMTP.
// Usa nodemailer como cliente SMTP.
//
// Por que nodemailer + Mailtrap:
...
// expuestas en el codigo fuente ni sean visibles en GitHub.

// Importamos nodemailer que es la libreria de Node.js para enviar correos via SMTP
import nodemailer from 'nodemailer';

// Creamos el transporter de nodemailer que es el objeto que sabe conectarse al servidor SMTP de Mailtrap
// process.env lee las variables del archivo .env que dotenv cargo al iniciar la aplicacion
// El operador || provee un valor de respaldo si la variable no esta definida en el .env
const transporter = nodemailer.createTransport({
    // host: el servidor SMTP de Mailtrap  sandbox intercepta los correos sin enviarlos a nadie real
    host:   process.env.MAILTRAP_HOST || 'sandbox.smtp.mailtrap.io',
    // port: el puerto SMTP que usa Mailtrap  puede ser 2525, 465 o 587 segun la configuracion
...
// Exportamos la funcion enviarCodigoRecuperacion que recibe el correo del usuario
// y el codigo de 6 digitos generado en resetCodes.js, y envia el email de recuperacion
// Retorna true si el correo se envio correctamente, false si hubo algun error
export async function enviarCodigoRecuperacion(destinatario, codigo) {
    try {
        ...
        const mailOptions = {
            // from: el nombre y direccion del remitente que vera el destinatario en su correo
            from:    `"Task App SENA" <${process.env.MAILTRAP_FROM || 'noreply@taskapp.sena.edu.co'}>`,
            // to: la direccion de correo del usuario que solicito la recuperacion
            to:      destinatario,
            // subject: el asunto del correo que aparece en la bandeja de entrada
            subject: 'Código de recuperación de contraseña  Task App',
            // text: version en texto plano para clientes de correo que no soportan HTML
            text: `Tu código de recuperación es: ${codigo}\n\nEste código vence en 15 minutos.\nSi no solicitaste este correo, ignóralo.`,
            // html: version con estilos visuales para que el correo se vea bien en Mailtrap
            html: `
        ...
        // Llamamos a transporter.sendMail que envia el correo al servidor SMTP de Mailtrap
        // En el panel de Mailtrap el correo aparece en el inbox para revisarlo visualmente
        await transporter.sendMail(mailOptions);
        // Retornamos true para indicar al controlador que el envio fue exitoso
        return true;

    } catch (error) {
        // Si el envio falla (credenciales incorrectas, Mailtrap caido, problema de red, etc.)
